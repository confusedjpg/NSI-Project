...
def confirm_transfer(info: dict) -> (typing.Tuple[tk.Toplevel, ttk.Progressbar, str] | None):
    ...
    confirm = askyesno(
        ...
        message = f"Veux-tu recevoir '{info['name']}' ({info['size']} octets)?",
        )
    # if he does,  prompt where to save the file
    if confirm:
        filetypes = [
            ("Nom original: " + info["name"], "*." + info["name"].split(".")[-1]), 
            ("All Files", "*.*"),
            ]
...
def send_file(ip: str):
    ...
    if file:
        def thread():
            ...
            # connect as a client to another user
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                sock.connect((ip, 6969))
                # send file info so other side knows what it's gonna deal with
                sock.sendall(json.dumps({"name":name, "size":size}).encode())
                counter = 0
                result = sock.recv(1024).decode()
                if int(result):
                    # use the received Public key to encrypt the Fernet key and send it
                    PADDING = padding.OAEP(
                        mgf = padding.MGF1(algorithm=hashes.SHA256()),
                        algorithm = hashes.SHA256(),
                        label = None,
                        )
                    KEY_PUBLIC = sock.recv(1024)
                    KEY_PUBLIC = serialization.load_pem_public_key(KEY_PUBLIC)
                    KEY_DECRYPT = KEY_PUBLIC.encrypt(FILE_KEY, PADDING)
                    sock.sendall(KEY_DECRYPT)

                    # finally, begin sending the file
                    while counter < size:
                        if size-counter >= PACKET_SIZE:
                            packet = content[counter:counter+PACKET_SIZE]
                            sock.sendall(packet)
                            counter += PACKET_SIZE
                        else:
                            packet = content[counter:size]
                            sock.sendall(packet) 
                            counter += len(packet)
                        
                        # TCP seems to be rate limited
                        # sending stuff faster without this micro sleep just fails
                        usleep(0.1)
        
        # putting all this into a thread so the program doesn't block
...
def refresh_users(info: list):
    """Update users (keep them alive or add them if they're new)."""
    message, name, ip = info[:-1]

    if message == 'Disponible' :
        # add a newly detected user to the online list, along with its info
        if not(ip in available_peers) and not(ip == MY_IP):  
            available_peers[ip] = {'name':name, 'button':None, 'here':True} 

        # or update its presence if they already exist
        elif not(ip == MY_IP):
            available_peers[ip]['here'] = True
            available_peers[ip]["name"] = name

# visually notify of no users
no_users = ttk.Label(frame, text="Aucun utilisateur n'est connecté :(", style="BW.TLabel")
def refresh_users_interface():
    """Update interface based on user information."""

    # will hold inactive users' containers to be deleted later
    # because changing a dict while it's iterating is a big no-no
    trash = []
    
    # UNTESTED
    if not ("!label" in frame.children and available_peers):
    ...
    # indefinitely loop over this function, every 500ms
